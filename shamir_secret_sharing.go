...
func newPolynomial(threshold uint32) polynomial_coeff {
	poly := make(polynomial_coeff, threshold)
...
func createRandomPolynomial(threshold uint32, masterSecretKey kyber.Scalar, groupOrder *big.Int) (poly polynomial_coeff, err error) {
	if groupOrder.Sign() < 0 {
		return polynomial_coeff{}, fmt.Errorf("group order is negative")
	}
...
func (p polynomial_coeff) eval(x kyber.Scalar) kyber.Scalar {
